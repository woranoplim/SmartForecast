...
# ---------- Load models ----------
lstm_model = load_model("lstm_META.h5", compile=False)
...
try:
    tcn_gru_model.load_state_dict(torch.load("META_tcn_gru.pt", map_location=device))
    tcn_gru_model.eval()
    tcn_available = True
...
# ---------- Predict ----------
lstm_pred_pct = lstm_model.predict(last_seq_lstm.reshape(1, seq_length_lstm, -1))[0]
trans_pred_pct = transformer_model.predict(last_seq_trans.reshape(1, seq_length_trans, -1))[0]

lstm_future_prices = last_close * (1 + lstm_pred_pct)
trans_future_prices = last_close * (1 + trans_pred_pct)

if tcn_available:
    input_tensor = torch.tensor(last_seq_tcn.reshape(1, seq_length_tcn, -1), dtype=torch.float32).to(device)
...
if tcn_available:
    results_df["TCN_GRU_Predicted_Price"] = tcn_future_prices

results_df.loc[-1] = {
    "Date": last_date.strftime("%Y-%m-%d"),
    "LSTM_Predicted_Price": last_close,
    "Transformer_Predicted_Price": last_close,
    "TCN_GRU_Predicted_Price": last_close if tcn_available else None,
}
...
print(f"✅ บันทึก {ticker}.json สำเร็จ")
